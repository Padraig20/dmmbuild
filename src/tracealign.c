...
static ESL_DSQ get_dsq_z(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int idx, int z);
static int make_digital_msa(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int nseq, const int *matuse, const int *matmap, int M, int alen, int optflags, ESL_MSA **ret_msa);
static int map_new_msa(F4_TRACE **tr, int nseq, int M, int optflags, int **ret_inscount, int **ret_matuse, int **ret_matmap, int *ret_alen);
static int make_text_msa(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int nseq, const int *matuse, const int *matmap, int M, int alen, int optflags, ESL_MSA **ret_msa);
static int annotate_rf(ESL_MSA *msa, int M, const int *matuse, const int *matmap);
static int annotate_posterior_probability(ESL_MSA *msa, F4_TRACE **tr, const int *matmap, int M, int optflags);
 ...
 */
int
f4_tracealign_MSA(const ESL_MSA *premsa, F4_TRACE **tr, int M, int optflags, ESL_MSA **ret_postmsa)
{
  ...
  if ((status = map_new_msa(tr, premsa->nseq, M, optflags, &inscount, &matuse, &matmap, &alen)) != eslOK) return status; //TODO
 
  if (optflags & f4_DIGITIZE) { if ((status = make_digital_msa(NULL, premsa, tr, premsa->nseq, matuse, matmap, M, alen, optflags, &msa)) != eslOK) goto ERROR; } //TODO
  else                        { if ((status = make_text_msa   (NULL, premsa, tr, premsa->nseq, matuse, matmap, M, alen, optflags, &msa)) != eslOK) goto ERROR; } //TODO

  if ((status = annotate_rf(msa, M, matuse, matmap))                          != eslOK) goto ERROR; //TODO
  if ((status = annotate_posterior_probability(msa, tr, matmap, M, optflags)) != eslOK) goto ERROR; //TODO

  if (optflags & f4_DIGITIZE) rejustify_insertions_digital(     msa, inscount, matmap, matuse, M); //TODO
  else                        rejustify_insertions_text   (abc, msa, inscount, matmap, matuse, M); //TODO
 ...
 */
static ESL_DSQ
get_dsq_z(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int idx, int z)
{
...
static int
make_digital_msa(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int nseq, const int *matuse, const int *matmap, int M, int alen, int optflags, ESL_MSA **ret_msa)
{
  ...
  for (idx = 0; idx < nseq; idx++)
      ...
      for (z = 0; z < tr[idx]->N; z++)
	{
	  ...
	  case f4T_I:
	    if ( !(optflags & f4_TRIM) || (tr[idx]->k[z] != 0 && tr[idx]->k[z] != M)) {
	  ...
	  case f4T_N:
	  case f4T_C:
	    if (! (optflags & f4_TRIM) && tr[idx]->i[z] > 0) {
	      msa->ax[idx][apos] = get_dsq_z(sq, premsa, tr, idx, z);
	      apos++;
	    }
	    break;
 ...
 * the <f4_ALL_CONSENSUS_COLS> option flag is set, though, all
 ...
 */
static int
map_new_msa(F4_TRACE **tr, int nseq, int M, int optflags, int **ret_inscount,
	    int **ret_matuse, int **ret_matmap, int *ret_alen)
{
  ...
  if (optflags & f4_ALL_CONSENSUS_COLS) esl_vec_ISet(matuse+1, M, TRUE); 
  ...
  for (idx = 0; idx < nseq; idx++)
	...
	{
      	  switch (tr[idx]->st[z]) {
	  case f4T_I:                                insnum[tr[idx]->k[z]]++; break;
	  case f4T_N: if (tr[idx]->st[z-1] == f4T_N) insnum[0]++;             break;
	  case f4T_C: if (tr[idx]->st[z-1] == f4T_C) insnum[M]++;             break;
	  case f4T_M: matuse[tr[idx]->k[z]] = TRUE;                           break;
	  case f4T_J: f4_Die("J state unsupported");
	  default:                                                            break;
	  }
	}
      for (k = 0; k <= M; k++) 
  ...
  if (optflags & f4_TRIM) { inscount[0] = inscount[M] = 0; }
 ...
 */
static int
make_text_msa(ESL_SQ **sq, const ESL_MSA *premsa, F4_TRACE **tr, int nseq, const int *matuse, const int *matmap, int M, int alen, int optflags, ESL_MSA **ret_msa)
{
  const ESL_ALPHABET *abc = (sq == NULL) ? premsa->abc : sq[0]->abc;
  ESL_MSA      *msa = NULL;
  int           idx;
 ...
 */
static int
annotate_posterior_probability(ESL_MSA *msa, F4_TRACE **tr, const int *matmap, int M, int optflags)
{
  ...
  for (idx = 0; idx < msa->nseq; idx++)
      ...
      for (z = 0; z < tr[idx]->N; z++)
	{
	  switch (tr[idx]->st[z]) {
	  case f4T_M: 
	    msa->pp[idx][matmap[tr[idx]->k[z]]-1] = f4_alidisplay_EncodePostProb(tr[idx]->pp[z]);  
	  ...
	  case f4T_I:
	    if ( !(optflags & f4_TRIM) || (tr[idx]->k[z] != 0 && tr[idx]->k[z] != M)) {
	      msa->pp[idx][apos] = f4_alidisplay_EncodePostProb(tr[idx]->pp[z]);  
	      apos++;
	    }
	    break;
  ...
  for (apos = 0; apos < msa->alen; apos++)
    if (matuse[apos]) msa->pp_cons[apos] = f4_alidisplay_EncodePostProb( totp[apos] / (double) matuse[apos]);
