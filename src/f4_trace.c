...
/* Function:  f4_trace_CreateWithPP()
 ...
 *            otherwise identical to <f4_trace_Create()>.
 */
F4_TRACE *
f4_trace_CreateWithPP(void)
{
  int       initial_nalloc    = 256;
  int       initial_ndomalloc = 16;
 ...
 */
int
f4_trace_GrowIndex(F4_TRACE *tr)
{
  void *p;
  int   status;
 ...
 */
int
f4_trace_GrowTo(F4_TRACE *tr, int N)
{
  int status;
  void *tmp;
 ...
 */
int
f4_trace_GrowIndexTo(F4_TRACE *tr, int ndom)
{
  void *p;
  int   status;
...
/* Function:  f4_trace_GetDomainCount()
 ...
 */
int
f4_trace_GetDomainCount(const F4_TRACE *tr, int *ret_ndom)
{
  int z;
  int ndom = 0;
  ...
  if (tr->ndom > 0) 
    ...
    for (z = 0; z < tr->N; z++)
      if (tr->st[z] == f4T_B) ndom++;
  }
  *ret_ndom = ndom;
  return eslOK;
}

/* Function:  f4_trace_GetStateUseCounts()
 * Incept:    SRE, Sun May 27 10:30:13 2007 [Janelia]
 *
 * Purpose:   Accumulate counts of each different state type in trace <tr>. 
 *
 *            <counts[]> is allocated for at least <f4T_NSTATETYPES>
 *            integers, indexed by statetype. Upon return,
 *            <counts[f4T_M]> contains the number of match states
 *            in the trace, for example.
 */
int
f4_trace_GetStateUseCounts(const F4_TRACE *tr, int *counts)
{
  int x,z;

  for (x = 0; x < f4T_NSTATETYPES; x++) counts[x] = 0;

  for (z = 0; z < tr->N; z++) {
    x = tr->st[z];
    if (x < 0 || x >= f4T_NSTATETYPES) ESL_EXCEPTION(eslEINVAL, "bad state type");
    counts[x]++;
  }
  return eslOK;
}

/* Function:  f4_trace_GetDomainCoords()
 ...
 *            <f4_trace_GetDomainCount()>, or caller can just loop
 ...
 */
int
f4_trace_GetDomainCoords(const F4_TRACE *tr, int which,
			 int *ret_i1, int *ret_i2, int *ret_k1, int *ret_k2)
{
   ...
   */
  if (tr->st[z] != f4T_M) ESL_XEXCEPTION(eslECORRUPT, "not a profile trace?");
  *ret_i1 = tr->i[z];
  *ret_k1 = tr->k[z];
   ...
   */
  for (; z < tr->N; z++)
    if (tr->st[z] == f4T_E) break;
  if (z == tr->N)         ESL_EXCEPTION(eslECORRUPT, "invalid trace: no E for a B");
  do { z--; } while (tr->st[z] == f4T_D); /* roll back over any D trailer */
  if (tr->st[z] != f4T_M) ESL_EXCEPTION(eslECORRUPT, "invalid trace: no M");
  *ret_i2 = tr->i[z];
  *ret_k2 = tr->k[z];
  return eslOK;
 ...
 */
int
f4_trace_Dump(FILE *fp, const F4_TRACE *tr, const F4_PROFILE *gm, const ESL_DSQ *dsq) /* replace void w/ F4_PROFILE */
{
  ...
  if (gm == NULL) 
    {		/* Yes, this does get used: during model construction. */ 
      ...
      for (z = 0; z < tr->N; z++) {
	fprintf(fp, "%1s  %4d %6d\n", 
		f4_hmm_DecodeStatetype(tr->st[z]),
		tr->k[z],
		tr->i[z]);
      } 
    } 
      ...
      for (z = 0; z < tr->N; z++) 
	{
	  if (z < tr->N-1) 
	    {
	      status = f4_profile_GetT(gm, tr->st[z], tr->k[z], tr->st[z+1], tr->k[z+1], &tsc); //TODO
	      if (status != eslOK) return status;
	    }
	  else tsc = 0.0f;

	  fprintf(fp, "%1s  %4d %6d  %8.4f", f4_hmm_DecodeStatetype(tr->st[z]),  tr->k[z], tr->i[z], tsc);
	  sc += tsc;
	  ...
	  if (dsq != NULL) {
	    ...
	    if (tr->st[z] == f4T_M) {
	      fprintf(fp, " %8.4f", f4P_MSC(gm, tr->k[z], xi));
	      sc += f4P_MSC(gm, tr->k[z], xi);
	      if (tr->pp != NULL) {
	    ...
	    else if (tr->st[z] == f4T_I) {
	      fprintf(fp, " %8.4f", f4P_ISC(gm, tr->k[z], xi));
	      sc += f4P_ISC(gm, tr->k[z], xi);
	      if (tr->pp != NULL) {
		fprintf(fp, " %8.4f", tr->pp[z]);
		accuracy += tr->pp[z];
	      }
	    ...
	    else if ((tr->st[z] == f4T_N && tr->st[z-1] == f4T_N) ||
		     (tr->st[z] == f4T_C && tr->st[z-1] == f4T_C) ||
		     (tr->st[z] == f4T_J && tr->st[z-1] == f4T_J))  {
	      fprintf(fp, " %8d", 0);
	      if (tr->pp != NULL) {
		fprintf(fp, " %8.4f", tr->pp[z]);
		accuracy += tr->pp[z];
 ...
 */
int
f4_trace_Compare(F4_TRACE *tr1, F4_TRACE *tr2, float pptol)
{
  int z,d;
 ...
 */
int 
f4_trace_Score(F4_TRACE *tr, ESL_DSQ *dsq, F4_PROFILE *gm, float *ret_sc)
{
  ...
  for (z = 0; z < tr->N-1; z++) {
    ...
    if      (tr->st[z] == f4T_M) sc += f4P_MSC(gm, tr->k[z], xi);
    else if (tr->st[z] == f4T_I) sc += f4P_ISC(gm, tr->k[z], xi);

    if ((status = f4_profile_GetT(gm, tr->st[z], tr->k[z], 
 ...
 *            
 *            In general, <pp> was created by <f4_GDecoding()> 
 ...
 */
int
f4_trace_SetPP(F4_TRACE *tr, const F4_GMX *pp)
{
  ...
  for (z = 0; z < tr->N; z++)
	...
	{
	  switch (tr->st[z]) {
	  case f4T_M:  tr->pp[z] = MMX(tr->i[z], tr->k[z]); break;
	  case f4T_I:  tr->pp[z] = IMX(tr->i[z], tr->k[z]); break;
	  case f4T_N:  tr->pp[z] = XMX(tr->i[z], f4G_N);    break;
	  case f4T_C:  tr->pp[z] = XMX(tr->i[z], f4G_C);    break;
	  case f4T_J:  tr->pp[z] = XMX(tr->i[z], f4G_J);    break;
...
f4_trace_AppendWithPP(F4_TRACE *tr, char st, int k, int i, float pp)
{
  ...
  case f4T_B:
  case f4T_E:
  case f4T_T: tr->i[tr->N] = 0; tr->pp[tr->N] = 0.0; tr->k[tr->N] = 0; break;
    /* Nonemitting, but in main model (k valid) */
  case f4T_D: tr->i[tr->N] = 0; tr->pp[tr->N] = 0.0; tr->k[tr->N] = k; break;
    /* Emitting states, with valid k position in model: */
  case f4T_M: 
  case f4T_I: tr->i[tr->N] = i; tr->pp[tr->N] = pp;  tr->k[tr->N] = k; break;
  default:    ESL_EXCEPTION(eslEINVAL, "no such state; can't append");
  }
...
f4_trace_Reverse(F4_TRACE *tr)
{
  ...
  for (z = 0; z < tr->N; z++)
    {
      if ( (tr->st[z] == f4T_N && tr->st[z+1] == f4T_N) ||
	   (tr->st[z] == f4T_C && tr->st[z+1] == f4T_C) ||
	   (tr->st[z] == f4T_J && tr->st[z+1] == f4T_J))
	{
  ...
  for (z = 0; z < tr->N; z++)
    {
      switch (tr->st[z]) {
      case f4T_B:
	if ((status = f4_trace_GrowIndex(tr)) != eslOK) goto ERROR;
	tr->tfrom[tr->ndom]   = z;
	tr->sqfrom[tr->ndom]  = 0;
 ...
 * Throws:    <eslEMEM> on allocation error.
 *
 * Xref:      J5/17: build.c::fake_tracebacks() becomes f4_trace_FauxFromMSA();
 ...
 */
int
p7_trace_Count(F4_HMM *hmm, ESL_DSQ *dsq, float wt, F4_TRACE *tr)
{
   ...
   * so it has no DI or ID transitions. -> Actually no, we actually
   * have DI and ID transitions in f4-HMMs, so we don't assume
   * that here, and we count them as well.
   */
	...
	if (st2 == f4T_M && k2 > 1)   /* wing-retracted B->DD->Mk path */
	  {
	    ...
	    for (ktmp = 1; ktmp < k2-1; ktmp++) 
	      hmm->t[ktmp][f4H_DD] += wt;
	    hmm->t[ktmp][f4H_DM] += wt;
	  }
	else  {
	  switch (st2) {
	  case f4T_M: hmm->t[0][f4H_MM] += wt; break;
	  case f4T_I: hmm->t[0][f4H_MI] += wt; break;
	  case f4T_D: hmm->t[0][f4H_MD] += wt; break;
	  default:     ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
     	...
     	switch (st2) {
	case f4T_M: hmm->t[k][f4H_MM] += wt; break;
	case f4T_I: hmm->t[k][f4H_MI] += wt; break;
	case f4T_D: hmm->t[k][f4H_MD] += wt; break;
	case f4T_E: hmm->t[k][f4H_MM] += wt; break; /* k==M. A local alignment would've been Mk->X->E. */
	default:     ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
	}
      }
      else if (st == f4T_I) {
	switch (st2) {
	case f4T_M: hmm->t[k][f4H_IM] += wt; break;
	case f4T_I: hmm->t[k][f4H_II] += wt; break;
	case f4T_E: hmm->t[k][f4H_IM] += wt; break; /* k==M. */
  case f4T_D: hmm->t[k][f4H_ID] += wt; break; /* Add this for f4-HMM */
	default:     ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
	}
      }
      else if (st == f4T_D) {
	switch (st2) {
	case f4T_M: hmm->t[k][f4H_DM] += wt; break;
	case f4T_D: hmm->t[k][f4H_DD] += wt; break;
	case f4T_E: hmm->t[k][f4H_DM] += wt; break; /* k==M. A local alignment would've been Dk->X->E. */
  case f4T_I: hmm->t[k][f4H_DI] += wt; break; /* Add this for f4-HMM */
	default:     ESL_EXCEPTION(eslEINVAL, "bad transition in trace");
	}
      }
 ...
 * Purpose:  Count a traceback into a count-based core HMM structure.
 *           (Usually as part of a model parameter re-estimation.)
 *           This is a special version of <f4_trace_Count()> for f4-HMMs,
