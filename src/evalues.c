 ...
 * 
 * Contents:
 *   1. f4_Calibrate():  model calibration wrapper 
 ...
 *            byp_bg  - BYPASS optimization: pass ptr to <F4_BG> if already known; 
 ...
 */
int
f4_Calibrate(F4_HMM *hmm, F4_BUILDER *cfg_b, ESL_RANDOMNESS **byp_rng, F4_BG **byp_bg, F4_PROFILE **byp_gm, F4_OPROFILE **byp_om)
{
  F4_BG          *bg     = (esl_byp_IsProvided(byp_bg)  ? *byp_bg  : NULL); 
  F4_PROFILE     *gm     = (esl_byp_IsProvided(byp_gm)  ? *byp_gm  : NULL); 
  F4_OPROFILE    *om     = (esl_byp_IsProvided(byp_om)  ? *byp_om  : NULL); 
  ...
  if (bg == NULL) {
    if ((bg = f4_bg_Create(hmm->abc)) == NULL)  ESL_XFAIL(eslEMEM, errbuf, "failed to allocate background");
  }
  ...
  if ((esl_byp_IsInternal(byp_gm) && ! esl_byp_IsProvided(byp_om)) || esl_byp_IsReturned(byp_gm)) {
    if  ( (gm     = f4_profile_Create(hmm->M, hmm->abc))          == NULL)  ESL_XFAIL(eslEMEM, errbuf, "failed to allocate profile");
    if  ( (status = f4_ProfileConfig(hmm, bg, gm, EvL, f4_LOCAL)) != eslOK) ESL_XFAIL(status,  errbuf, "failed to configure profile");
  }

  if (om == NULL) {
    if ((om     = f4_oprofile_Create(hmm->M, hmm->abc)) == NULL) ESL_XFAIL(eslEMEM, errbuf, "failed to create optimized profile");
    if ((status = f4_oprofile_Convert(gm, om))         != eslOK) ESL_XFAIL(status,  errbuf, "failed to convert to optimized profile");
  }

  /* The calibration steps themselves */
  if ((status = f4_Lambda(hmm, bg, &lambda))                          != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine lambda");
  if ((status = f4_MSVMu    (r, om, bg, EmL, EmN, lambda, &mmu))      != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine msv mu");
  if ((status = f4_ViterbiMu(r, om, bg, EvL, EvN, lambda, &vmu))      != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine vit mu");
  if ((status = f4_Tau      (r, om, bg, EfL, EfN, lambda, Eft, &tau)) != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine fwd tau");

  /* Store results */
  hmm->evparam[f4_MLAMBDA] = om->evparam[f4_MLAMBDA] = lambda;
  hmm->evparam[f4_VLAMBDA] = om->evparam[f4_VLAMBDA] = lambda;
  hmm->evparam[f4_FLAMBDA] = om->evparam[f4_FLAMBDA] = lambda;
  hmm->evparam[f4_MMU]     = om->evparam[f4_MMU]     = mmu;
  hmm->evparam[f4_VMU]     = om->evparam[f4_VMU]     = vmu;
  hmm->evparam[f4_FTAU]    = om->evparam[f4_FTAU]    = tau;
  hmm->flags              |= f4H_STATS;

  if (gm != NULL) {
    gm->evparam[f4_MLAMBDA] = lambda;
    gm->evparam[f4_VLAMBDA] = lambda;
    gm->evparam[f4_FLAMBDA] = lambda;
    gm->evparam[f4_MMU]     = mmu;
    gm->evparam[f4_VMU]     = vmu;
    gm->evparam[f4_FTAU]    = tau;
  }
    
  if (byp_rng != NULL) *byp_rng = r;  else esl_randomness_Destroy(r); /* bypass convention: no-op if rng was provided.*/
  if (byp_bg  != NULL) *byp_bg  = bg; else f4_bg_Destroy(bg);         /* bypass convention: no-op if bg was provided. */
  if (byp_gm  != NULL) *byp_gm  = gm; else f4_profile_Destroy(gm);    /* bypass convention: no-op if gm was provided. */
  if (byp_om  != NULL) *byp_om  = om; else f4_oprofile_Destroy(om);   /* bypass convention: no-op if om was provided. */
  return eslOK;

 ERROR:
  if (! esl_byp_IsProvided(byp_rng)) esl_randomness_Destroy(r);
  if (! esl_byp_IsProvided(byp_bg))  f4_bg_Destroy(bg);
  if (! esl_byp_IsProvided(byp_gm))  f4_profile_Destroy(gm);
  if (! esl_byp_IsProvided(byp_om))  f4_oprofile_Destroy(om);
  return status;
}
/*---------------------- end, wrapper API -----------------------*/
 ...
 */
int
f4_Lambda(F4_HMM *hmm, F4_BG *bg, double *ret_lambda)
{
  double H = f4_MeanMatchRelativeEntropy(hmm, bg);
 ...
 */
int
f4_MSVMu(ESL_RANDOMNESS *r, F4_OPROFILE *om, F4_BG *bg, int L, int N, double lambda, double *ret_mmu)
{
  F4_OMX  *ox      = f4_omx_Create(om->M, 0, 0); /* DP matrix: 1 row version */
  ESL_DSQ *dsq     = NULL;
  double  *xv      = NULL;
  int      i;
  float    sc, nullsc;
  ...
  f4_oprofile_ReconfigLength(om, L);
  f4_bg_SetLength(bg, L);

  for (i = 0; i < N; i++)
    {
      if ((status = esl_rsq_xfIID(r, bg->f, om->abc->K, L, dsq)) != eslOK) goto ERROR;
      if ((status = f4_bg_NullOne(bg, dsq, L, &nullsc))          != eslOK) goto ERROR;   

      status = f4_MSVFilter(dsq, L, om, ox, &sc);
 ...
 */
int
f4_ViterbiMu(ESL_RANDOMNESS *r, F4_OPROFILE *om, F4_BG *bg, int L, int N, double lambda, double *ret_vmu)
{
  F4_OMX  *ox      = f4_omx_Create(om->M, 0, 0); /* DP matrix: 1 row version */
  ESL_DSQ *dsq     = NULL;
  double  *xv      = NULL;
  ...
  for (i = 0; i < N; i++)
    {
      ...
      status = f4_ViterbiFilter(dsq, L, om, ox, &sc);
      if (status == eslERANGE) { sc = maxsc; status = eslOK; }
      if (status != eslOK)     goto ERROR;
 ...
 *            lambda : expected slope of the exponential tail (from f4_Lambda())
 ...
 */
int
f4_Tau(ESL_RANDOMNESS *r, F4_OPROFILE *om, F4_BG *bg, int L, int N, double lambda, double tailp, double *ret_tau)
{
  F4_OMX  *ox      = f4_omx_Create(om->M, 0, L);     /* DP matrix: for ForwardParser,  L rows */
  ESL_DSQ *dsq     = NULL;
  double  *xv      = NULL;
  float    fsc, nullsc;		                  
  double   gmu, glam;
  ...
  for (i = 0; i < N; i++)
    {
      if ((status = esl_rsq_xfIID(r, bg->f, om->abc->K, L, dsq)) != eslOK) goto ERROR;
      if ((status = f4_ForwardParser(dsq, L, om, ox, &fsc))      != eslOK) goto ERROR; //TODO
      if ((status = f4_bg_NullOne(bg, dsq, L, &nullsc))          != eslOK) goto ERROR;
