...
f4_prior_CreateAmino(void)
{
 ...
 ERROR:
  if (pri != NULL) f4_prior_Destroy(pri);
  return NULL;
}

/* Function:  f4_prior_CreateNucleic()
 *
 ...
 *            observed count data.
 *
 * Returns:   a pointer to the new <F4_PRIOR> structure.
 */
F4_PRIOR *
f4_prior_CreateNucleic(void)
{
  int status;
  F4_PRIOR *pri = NULL;
  int q;
...
*/
  ...
  pri->ti->alpha[0][2] = 0.1;  // TID -  added arbitrary non-zero value
 ...
 */
F4_PRIOR *
f4_prior_CreateLaplace(const ESL_ALPHABET *abc)
{
  F4_PRIOR *pri = NULL;
  int        status;
 ...
 * 
 * TODO: Will be removed in the future, as it is not used in f4.
 */
int
p7_ParameterEstimation(F4_HMM *hmm, const F4_PRIOR *pri)
{
  int   k;
  double c[f4_MAXABET];
  double p[f4_MAXABET];
  ...
  if (pri==NULL) return f4_hmm_Renormalize(hmm);
...
f4_ParameterEstimation(F4_HMM *hmm, const F4_PRIOR *pri)
{
   ...
   */
  for (k = 0; k <= hmm->M; k++) {
    hmm->t[k][f4H_MM] = hmm->tp[k][f4H_GAMMA];
    hmm->t[k][f4H_MI] = hmm->tp[k][f4H_ALPHA];
    hmm->t[k][f4H_MD] = hmm->tp[k][f4H_DELTA];
  }
  hmm->t[hmm->M][f4H_MD] = 0.0;
  esl_vec_FNorm(hmm->t[hmm->M], 3);
   ...
   */
  for (k = 0; k <= hmm->M; k++) {
    hmm->t[k][f4H_II] = hmm->tp[k][f4H_BETA] + hmm->tp[k][f4H_BETAP] * hmm->tp[k][f4H_ALPHA];
    hmm->t[k][f4H_IM] = (1 - hmm->tp[k][f4H_BETA]) * hmm->tp[k][f4H_GAMMA];
    hmm->t[k][f4H_ID] = hmm->tp[k][f4H_BETAP] * hmm->tp[k][f4H_DELTA];
  }
   ...
   */
  for (k = 1; k < hmm->M; k++) {
    hmm->t[k][f4H_DD] = hmm->tp[k][f4H_EPSILON] + hmm->tp[k][f4H_EPSILONP] * hmm->tp[k][f4H_DELTA];
    hmm->t[k][f4H_DI] = hmm->tp[k][f4H_EPSILONP] * hmm->tp[k][f4H_ALPHA];
    hmm->t[k][f4H_DM] = hmm->tp[k][f4H_EPSILONP] * hmm->tp[k][f4H_GAMMA];
  }
  hmm->t[0][f4H_DM] = hmm->t[hmm->M][f4H_DM] = 1.0;
  hmm->t[0][f4H_DD] = hmm->t[hmm->M][f4H_DD] = 0.0;
