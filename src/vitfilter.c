 ...
 */
int
f4_ViterbiFilter(const ESL_DSQ *dsq, int L, const F4_OPROFILE *om, F4_OMX *ox, float *ret_sc)
{
  ...
  int Q        = f4O_NQW(om->M);   /* segment length: # of vectors                              */
  ...
  if (om->mode != f4_LOCAL && om->mode != f4_UNILOCAL) ESL_EXCEPTION(eslEINVAL, "Fast filter only works for local alignment");
  ox->M   = om->M;
  ...
  for (i = 1; i <= L; i++)
    {
      ...
      xN = xN + om->xw[f4O_N][f4O_LOOP];
      xC = ESL_MAX(xC + om->xw[f4O_C][f4O_LOOP], xE + om->xw[f4O_E][f4O_MOVE]);
      xJ = ESL_MAX(xJ + om->xw[f4O_J][f4O_LOOP], xE + om->xw[f4O_E][f4O_LOOP]);
      xB = ESL_MAX(xJ + om->xw[f4O_J][f4O_MOVE], xN + om->xw[f4O_N][f4O_MOVE]);
...
#if eslDEBUGLEVEL > 0
      if (ox->debugging) f4_omx_DumpVFRow(ox, i, xE, 0, xJ, xB, xC);   
#endif
  ...
  if (xC > -32768)
    {
      *ret_sc = (float) xC + (float) om->xw[f4O_C][f4O_MOVE] - (float) om->base_w;
